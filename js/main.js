...
// 요소 수평 슬라이드 만들기 (AWARDS)
new Swiper('.awards .swiper-container', {
  autoplay: true,
  loop: true,
  spaceBetween: 30, // 슬라이드 사이의 여백 30px
  slidesPerView: 5, // 한번에 보여질 슬라이드 개수 
  navigation: {
...
promotionToggleBtn.addEventListener('click', function() {
  isHidPromotion = !isHidPromotion;  // (false => true) 반대 되는 값 반환
  if(isHidPromotion) {
    // 숨김 처리!
...
function floatingObject(selector, delay, size) {
  ...
  gsap.to(selector, // 선택자
     ...
     {  // 옵션
        y: size,
        repeat: -1, // 무한 반복
        yoyo: true, // 한번 재생된 에니메이션을 다시 거꾸로 재생
